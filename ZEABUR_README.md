...
4. **Frontend:** deploy with the **root `Dockerfile`** (port **8080**), not plain static hosting — Caddy serves `/`, `/univer/`, `/HBMP_One/`, `/HBMP_DOCS_PLATFORM/`, `/HBMP_AgentBot/`, `/Google/`, `/app_manager/`, `/hbmp_form_manager/`, `/ivvychainv2/`, `/OpenStreetMaps/`, `/ImageProcessing/`, and `/mcp_server/`. See **[DEPLOY_ZEABUR.md](./DEPLOY_ZEABUR.md)**.
