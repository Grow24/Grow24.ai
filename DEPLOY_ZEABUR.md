...
**Default build profile is `core`** (skips Univer / Mxgraph / ivvychain — avoids OOM on Zeabur Dev 2vCPU/4GB). Core still builds **HBMP_DOCS_PLATFORM** (UI + API in this container) and **HBMP_AgentBot** (static UI + local API stub in this same container). Set `GOOGLE_KEY` on this frontend service for AgentBot chat.
...
3. Add environment variables (build-time where noted):
   ...
   - **AgentBot at `/HBMP_AgentBot/` (runtime, same container):** `GOOGLE_KEY=<Gemini API key>` — UI and API both run in this image (`scripts/agentbot-stub.cjs`). Do not point AgentBot at any other host.
